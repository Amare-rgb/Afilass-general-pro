...
// Supports local public folder paths (e.g., "/afilas.jpg") or full URLs.
// The image will automatically resize & scale (object-cover) inside the card.
// =========================================================================
export const HOSPITAL_HERO_IMAGE_PATH = "/afilas.jpg";

import { useLanguage } from "@/contexts/LanguageProvider";
import Link from "next/link";
import { ArrowDown, ChevronRight, Users, Stethoscope } from "lucide-react";
...
export function HospitalHero() {
  ...
  // Intersection observer for text enter/exit animation on desktop
  useEffect(() => {
  ...
  return (
    // Sticky slide on desktop – full viewport height. Normal relative flow on mobile.
    <div
      ref={heroRef}
      className="relative sm:sticky sm:top-0 h-screen min-h-screen overflow-hidden flex items-center py-0 bg-gradient-to-br from-teal-800 via-teal-900 to-slate-950"
    >
...
<div className="hidden lg:flex items-center justify-center lg:justify-end">
  <div className="relative w-full max-w-xl lg:max-w-2xl">
    <div className="relative aspect-square rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center overflow-hidden shadow-2xl">
      <img
        src={HOSPITAL_HERO_IMAGE_PATH}
        alt="Afilas General Hospital"
        className="w-full h-full object-cover rounded-2xl"
      />
      {/* Shade overlay */}
<div className="absolute inset-0 bg-gradient-to-b from-black/30 to-transparent pointer-events-none rounded-2xl" />
    </div>
