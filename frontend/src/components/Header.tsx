...
export function Header() {
  ...
  // Afilas Group branches (translated)
...
useEffect(() => {
  const handleScroll = () => {
    const currentScrollY = window.scrollY;
    // Only hide after scrolling down 50px to avoid flicker at the very top
...
>
        {/* 
          ...
          ============================================================
          isSolid = true  -> solid (always)
          isSolid = false -> transparent (only on /, /blogs, /aboutUs at the top)
        */}
        <div
        ...
        <nav className="relative flex w-full items-center justify-between gap-2 px-2 py-2 sm:px-4 lg:px-6">
                  ...
                  {(() => {
                    ...
                    return (
                      <IconComponent className={`h-3 w-3 sm:h-5 sm:w-5 flex-shrink-0 ${
                        isSolid ? "text-teal-600 dark:text-teal-400" : "text-white"
                      }`} />
                    );
                  })()}
          ...
          <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-center">
            <div className="flex items-center gap-1 xl:gap-2">
              ...
              <div className="relative" ref={groupDropdownRef}>
                <button
                  ...
                  className={`flex items-center rounded-lg px-2.5 py-1.5 text-sm font-medium transition-colors whitespace-nowrap ${
                    isSolid
                      ? "text-slate-800 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"
                      : "text-white/90 hover:bg-white/20 hover:text-white"
                  } ${groupDropdownOpen ? (isSolid ? "bg-slate-100 dark:bg-slate-800" : "bg-white/20") : ""}`}
                  aria-haspopup="menu"
                ...
                {groupDropdownOpen && (
                  <div className={`absolute left-0 top-full z-50 mt-3 w-[650px] max-w-[90vw] rounded-2xl p-6 transition-all duration-300 shadow-2xl ${
                    isSolid
                      ? "bg-white/95 backdrop-blur-xl border border-slate-200/80 dark:bg-slate-900/95 dark:border-slate-700/80"
                      : "bg-white/10 backdrop-blur-2xl border border-white/25"
                  }`} role="menu">
                    <div className="text-center mb-5">
                      <p className={`text-sm font-semibold tracking-wide ${isSolid ? "text-slate-800 dark:text-slate-200" : "text-white"}`}>
                        {t("nav.group_dropdown_title") || "Explore Our Divisions"}
                      </p>
                      <div className={`w-10 h-0.5 mx-auto mt-2 rounded-full ${isSolid ? "bg-teal-500" : "bg-white/50"}`}></div>
                    </div>
                        ...
                        return (
                          ...
                          >
                            <div className={`mb-3 flex h-14 w-14 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110 ${
                              isSolid ? "bg-slate-100 dark:bg-slate-800 group-hover:shadow-lg" : "bg-white/20 group-hover:bg-white/30"
                            }`}>
                              <IconComponent className={`h-7 w-7 ${isSolid ? "text-slate-700 dark:text-slate-300" : "text-white"}`} />
                            </div>
                            <div className="space-y-0.5">
                              <p className={`text-sm font-bold uppercase tracking-wide ${isSolid ? "text-slate-800 dark:text-slate-200" : "text-white"}`}>
                                {item.label}
                              </p>
                              <p className={`text-[10px] max-w-[140px] mx-auto leading-relaxed ${isSolid ? "text-slate-500 dark:text-slate-400" : "text-white/80"}`}>
                                {item.description}
                              </p>
                              {isSelected && (
              ...
              {/* CONDITIONAL: only on branch pages */}
              {isBranchPage && (
                <>
                  <Link
                    href="/services"
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
            ...
            <div className="relative" ref={languageDropdownRef}>
              <button
                ...
                } ${languageDropdownOpen ? (isSolid ? "bg-slate-100 dark:bg-slate-800" : "bg-white/20") : ""}`}
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-0.5 sm:gap-1 rounded-lg border px-1 sm:px-2 py-0.5 sm:py-1 text-[9px] sm:text-xs lg:text-sm font-medium transition-colors ${
                    isSolid
                      ? "border-slate-200 text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                      : "border-white/30 text-white hover:bg-white/20"
                  } ${themeDropdownOpen ? (isSolid ? "bg-slate-100 dark:bg-slate-800" : "bg-white/20") : ""}`}
            ...
            <div className="relative" ref={appointmentDropdownRef}>
              <button
                ...
                } ${appointmentDropdownOpen ? (isSolid ? "ring-2 ring-teal-400" : "ring-2 ring-white/50") : ""}`}
                aria-haspopup="true"
...
if (typeof window !== 'undefined') {
  const setHeaderOffset = () => {
    try {
      ...
      // Base space: topbar + header + 8px
      let total = topbarH + headerH + 8;

      // 🔹 ADD EXTRA SPACE for /diagnostics and /pharma
      const path = window.location.pathname;
      if (path.startsWith('/diagnostics') || path.startsWith('/pharma')) {
        total += 100; // Add 40px extra space
      }
