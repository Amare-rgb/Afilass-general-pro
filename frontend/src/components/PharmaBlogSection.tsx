// frontend/src/components/PharmaBlogSection.tsx
"use client";
...
export function PharmaBlogSection() {
  ...
  if (error) {
    return (
      <div className="py-12 text-center">
        ...
        <button onClick={loadBlogs} className="mt-2 text-green-600 hover:underline">
          Try Again
        </button>
      </div>
    );
        ...
        <div className="flex items-center justify-between mb-12">
          <div>
            ...
            <p className="text-lg text-foreground/70">
              Latest pharmaceutical news, drug research, and industry updates
            </p>
          </div>
          <Link
            href="/blogs?location=pharma"
              ...
              <Link href={`/blogs/${blog.slug}`}>
                <div className="relative h-48 bg-slate-900 overflow-hidden">
                  {blog.image ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-green-900/60 to-slate-900 flex items-center justify-center">
                      <span className="text-slate-400 text-sm font-semibold">{blog.category}</span>
                    </div>
                  )}
                  <div className="absolute top-3 left-3">
                    <span className="px-2.5 py-1 rounded-md bg-green-600/90 text-white text-xs font-bold">
                      {blog.category}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="text-lg font-bold line-clamp-2 hover:text-green-600 transition-colors">
                    {blog.title}
                  </h3>
