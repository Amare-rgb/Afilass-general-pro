...
export default function DiagnosisBookingPage() {
  ...
  const nextStep = async () => {
    const step1Fields: (keyof DiagnosisBookingData)[] = ['patientName', 'patientPhone', 'patientEmail'];
  ...
  const onSubmit = async (data: DiagnosisBookingData) => {
    ...
    try {
      ...
      if (payData.success && payData.checkout_url) {
        ...
        toast.success(isAm ? 'የምርመራ ቀጠሮዎ ተይዟል! 🎉' : 'Diagnostic appointment booked successfully! 🎉');
      }
    } catch (error: any) {
