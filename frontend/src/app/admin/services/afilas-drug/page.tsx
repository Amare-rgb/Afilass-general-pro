...
export default function AdminServicesAfilasDrugPage() {
  const { t } = useLanguage();
  ...
  async function loadServices() {
    ...
    try {
      ...
      console.log(`✅ Loaded ${servicesData.length} products for ${LOCATION}`);
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(editingService ? 'Product updated successfully!' : 'Product created successfully!');
      handleCloseModal();
      await loadServices();
    } catch (error: any) {
      setUploading(false);
  ...
  const getCategoryColor = (category: string) => {
    const colors: { [key: string]: string } = {
      'Prescription Drugs': 'bg-red-100 text-red-700',
      'Over-The-Counter': 'bg-blue-100 text-blue-700',
      'Antibiotics': 'bg-green-100 text-green-700',
      'Pain Relievers': 'bg-orange-100 text-orange-700',
      'Antivirals': 'bg-purple-100 text-purple-700',
      'Vaccines': 'bg-cyan-100 text-cyan-700',
      'Insulin & Diabetes': 'bg-pink-100 text-pink-700',
      'Cardiovascular': 'bg-rose-100 text-rose-700',
      'Respiratory': 'bg-indigo-100 text-indigo-700',
      'Dermatological': 'bg-yellow-100 text-yellow-700',
      ...
      'Nutraceuticals': 'bg-emerald-100 text-emerald-700'
    };
    ...
    <>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="text-sm text-gray-500">
            {filteredServices.length} product{filteredServices.length !== 1 ? 's' : ''}
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">
            {searchTerm || categoryFilter ? 'Try adjusting your filters' : 'Click "Add Product" to create one'}
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredServices.map((service) => (
            <div key={service.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
              {/* ✅ FIXED: Product Image with full URL */}
      ...
      {!loading && filteredServices.length > 0 && (
        <div className="mt-4 text-xs text-gray-400 flex items-center justify-end">
          {filteredServices.length} product{filteredServices.length !== 1 ? 's' : ''} in {LOCATION}
        </div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between rounded-t-xl">
              <h3 className="text-sm font-semibold text-gray-800">
                {editingService ? '✏️ Edit Product' : '➕ New Product'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-3">
              ...
              {/* ✅ FIXED: Action Buttons - White color with "Add Product" text */}
                ...
                >
                  {uploading ? (
                    ...
                    <>
                      ...
                      {editingService ? 'Update Product' : 'Add Product'}
                    </>
                  )}
                </button>
                <button
