...
export default function AfilasDiagnosisAppointmentsPage() {
  const { t } = useLanguage();
  ...
  async function load() {
    ...
    try {
      ...
      const response: any = await api.getAppointments(
        filter ? { status: filter, location: LOCATION_NAME } : { location: LOCATION_NAME },
        true
      );
      ...
      const homeVisits = updatedAppointments.filter(a => a.visitType?.toUpperCase() === 'HOME').length;
      
      // 🔥 Updated stats with Confirmed and Missed
      setStats({
  ...
  // 🔥 Approve appointment - sends CONFIRMED status
  async function approveAppointment(id: string) {
    ...
    try {
      ...
      if (response && typeof response === 'object') {
        if ('notifications' in response && response.notifications) {
          const notif = response.notifications;
          if (notif.emailSent) {
            toast.success('✅ Approval email sent to patient');
          }
          if (notif.smsSent) {
            toast.success('📱 Approval SMS sent to patient');
          }
          if (!notif.emailSent && !notif.smsSent) {
            toast.info('Appointment approved. Notifications will be sent.');
          }
        } else {
  ...
  // 🔥 Reject appointment - sends CANCELLED status
    ...
    try {
      ...
      if (response && typeof response === 'object') {
        if ('notifications' in response && response.notifications) {
          const notif = response.notifications;
          if (notif.emailSent) {
            toast.success('✅ Rejection email sent to patient');
          }
          if (notif.smsSent) {
            toast.success('📱 Rejection SMS sent to patient');
          }
          if (!notif.emailSent && !notif.smsSent) {
            toast.info('Appointment rejected. Notifications will be sent.');
          }
        } else {
  ...
  // 🔥 Complete appointment - sends COMPLETED status
  ...
  async function remove(id: string) {
    ...
    try {
      ...
      setSuccess('🗑️ Appointment deleted successfully');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 🔥 Stats Cards - NO Home Visits Card */}
                  ...
                  return (
                    <tr key={appointment.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors last:border-0 ${isPast && !isMissed ? 'bg-orange-50/30' : ''} ${isMissed ? 'bg-gray-50 opacity-60' : ''} ${isHomeVisit ? 'bg-blue-50/30' : ''}`}>
                      <td className="py-3 px-3">
                        ...
                        {isMissed && (
                          <span className="inline-block mt-1 text-[10px] font-semibold text-gray-600 bg-gray-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                            ⏰ Missed
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          {/* Status display for non-pending appointments */}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh] space-y-4">
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="border-b border-gray-100 dark:border-gray-700 pb-3">
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">📍 GPS Pin / Location</p>
