    ...
    <>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{t('admin.appointments.subtitle')}</p>
        </div>
                  ...
                  <tr key={appointment.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors last:border-0 align-top">
                    ...
                    <td className="py-3 px-5">
                      ...
                      {appointment.doctor?.title && (
                        <div className="text-xs text-gray-500 ml-5">{appointment.doctor.title}</div>
                      )}
                    </td>
                    <td className="py-3 px-5">
                      <div className="text-gray-800">{formatDate(appointment.appointmentDate)}</div>
                      {appointment.reminderSentAt && (
                        <div className="text-xs text-green-600">✅ {t('admin.appointments.reminder_sent')}</div>
                      )}
                    </td>
                    <td className="py-3 px-5">
                      <select
      ...
      {!loading && appointments.length > 0 && (
        <div className="mt-4 text-xs text-gray-400">
          {t('admin.appointments.showing')} {appointments.length} {t('admin.appointments.appointment')}{appointments.length !== 1 ? 's' : ''}
        </div>
      )}
    </>
  );
