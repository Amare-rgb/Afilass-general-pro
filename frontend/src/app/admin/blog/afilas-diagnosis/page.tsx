...
export default function AdminBlogAfilasDiagnosisPage() {
  const { t } = useLanguage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: Remove mediaType from submit data - backend doesn't store it
      const submitData = {
        title: formData.title.trim(),
        content: formData.content.trim(),
        excerpt: formData.excerpt.trim(),
        category: formData.category,
        location: formData.location || 'Afilas Diagnosis Center',
        tags: formData.tags,
        image: mediaType === 'image' ? imageUrl : '',
        videoUrl: mediaType === 'video' ? videoUrl : '',
        isPublished: formData.isPublished,
        author: 'Admin', // Required by backend
        // DO NOT include mediaType here - it's not stored in the database
      };
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      'Diagnostic News': 'bg-blue-100 text-blue-700',
      'Medical Research': 'bg-purple-100 text-purple-700',
      'Test Procedures': 'bg-green-100 text-green-700',
      'Health Screening': 'bg-teal-100 text-teal-700',
      'Technology': 'bg-indigo-100 text-indigo-700',
      'Patient Education': 'bg-yellow-100 text-yellow-700',
      'Lab Updates': 'bg-orange-100 text-orange-700',
      'Preventive Care': 'bg-cyan-100 text-cyan-700'
    };
      ...
      <div className="flex items-center justify-end mb-6 flex-wrap gap-4">
        <div className="flex gap-3 flex-wrap">
          ...
          <button
            ...
            className="rounded-lg bg-white border border-gray-300 hover:bg-gray-50 hover:border-gray-400 text-gray-700 px-4 py-2 text-sm transition-colors flex items-center gap-2 shadow-sm"
          >
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="relative">
            <select
              ...
              className="pl-3 pr-8 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-400 focus:border-transparent appearance-none bg-white text-sm"
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          ...
          {!searchTerm && !categoryFilter && !statusFilter && (
            <button
              onClick={() => handleOpenModal()}
              className="rounded-lg bg-white border border-gray-300 hover:bg-gray-50 hover:border-gray-400 text-gray-700 px-5 py-2 text-sm transition-colors shadow-sm"
            >
              + New Post
            </button>
          )}
            ...
            <div key={post.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="relative h-48 bg-gray-100">
                ...
                <div className="absolute bottom-2 left-2">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                    post.videoUrl ? 'bg-gray-700 text-white' : 'bg-gray-600 text-white'
                  }`}>
              ...
              <div className="p-4">
                ...
                <div className="flex gap-1 mt-3 pt-3 border-t border-gray-100">
                  ...
                  <button
                    ...
                    className="flex-1 text-[10px] text-gray-600 hover:text-gray-800 font-medium hover:bg-gray-100 px-2 py-1 rounded transition-colors"
                  >
                    Edit
                  </button>
                  <button
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-3">
                ...
                <div data-error={!!formErrors.image && touched.image}>
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors ${
                      previewImage ? 'border-gray-400 bg-gray-50' : 'border-gray-300 hover:border-gray-400'
                    }`}
                  >
                    {previewImage ? (
                      <div className="relative">
              ...
              <div data-error={!!formErrors.tags && touched.tags}>
                ...
                <div className="flex gap-1.5">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-white border border-gray-300 hover:bg-gray-50 hover:border-gray-400 rounded-lg transition-colors text-sm text-gray-700"
                  >
                    Add
                  </button>
                </div>
              ...
              {/* Action Buttons - ALL WHITE WITH HOVER EFFECTS */}
              <div className="flex gap-2 pt-2 border-t border-gray-100">
                <button
                  type="submit"
                  disabled={uploadingMedia || isSubmitting}
                  className="flex-1 rounded-lg bg-white border border-gray-300 hover:bg-gray-50 hover:border-gray-400 text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium px-4 py-2 transition-colors flex items-center justify-center gap-2 shadow-sm"
                >
                ...
                <button
                  type="button"
                  onClick={handleCloseModal}
                  className="flex-1 rounded-lg bg-white border border-gray-300 hover:bg-gray-50 hover:border-gray-400 text-gray-700 text-sm font-medium px-4 py-2 transition-colors"
                >
                  Cancel
                </button>
              </div>
